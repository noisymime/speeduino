...
void calculateSecondarySpark(void)
{
  ...
  if (!isFixedTimingOn())
  {
    if(configPage10.spark2Mode == SPARK2_MODE_MULTIPLY)
    {
      ...
      uint8_t percentage = lookupSpark2();
      //Spark 2 table is treated as a % value. Table 1 and 2 are multiplied together and divided by 100
      int16_t combinedAdvance = div100((int16_t)((int16_t)percentage * (int16_t)currentStatus.advance1));
      ...
      currentStatus.advance2 = max(MIN_ADVANCE, (int8_t)((int16_t)percentage-INT16_C(OFFSET_IGNITION)));
    }
