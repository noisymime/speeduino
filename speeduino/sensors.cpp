...
static inline int16_t fastMap10Bit(uint16_t value, int16_t rangeMin, int16_t rangeMax) {
  ...
  uint16_t fromStartOfRange = (uint16_t)(((uint32_t)value * range) >> 10U);
...
static uint16_t MAPlast; /**< The previous MAP reading */
static uint32_t MAP_time; //The time the MAP sample was taken
static uint32_t MAPlast_time; //The time the previous MAP sample was taken
...
void readTPS(bool useFilter)
{
  ...
  else
  {
    ...
    tempADC = 255U - tempADC; //Reverse the ADC values
    uint8_t tempTPSMax = 255U - configPage2.tpsMax;
    uint8_t tempTPSMin = 255U - configPage2.tpsMin;
