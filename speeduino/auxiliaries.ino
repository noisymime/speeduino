...
void vvtControl()
{
  if( (configPage6.vvtEnabled == 1) && (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)) )
  {
    ...
    {
      //Lookup VVT duty based on either MAP or TPS
      if(configPage6.vvtLoadSource == VVT_LOAD_TPS) { currentStatus.vvt1TargetAngle = scaleCrankAngle(get3DTableValue(&vvtTable, currentStatus.TPS, currentStatus.RPM)); }
      else { currentStatus.vvt1TargetAngle = scaleCrankAngle(get3DTableValue(&vvtTable, currentStatus.MAP, currentStatus.RPM)); }
      ...
      if (configPage10.vvt2Enabled == 1) // same for VVT2 if it's enabled
      {
        if(configPage6.vvtLoadSource == VVT_LOAD_TPS) { currentStatus.vvt2TargetAngle = scaleCrankAngle(get3DTableValue(&vvt2Table, currentStatus.TPS, currentStatus.RPM)); }
        else { currentStatus.vvt2TargetAngle = scaleCrankAngle(get3DTableValue(&vvt2Table, currentStatus.MAP, currentStatus.RPM)); }
        ...
        // safety check that the cam angles are ok. The engine will be totally undriveable if the cam sensor is faulty and giving wrong cam angles, so if that happens, default to 0 duty.
        // This also prevents using zero or negative current angle values for PID adjustment, because those don't work in integer PID.
        if ( currentStatus.vvt2Angle <= scaleCrankAngle(configPage10.vvtCLMinAng) || currentStatus.vvt2Angle > scaleCrankAngle(configPage10.vvtCLMaxAng) )
        {
          currentStatus.vvt2Duty = 0;
          vvt2_pwm_value = halfPercentage(currentStatus.vvt2Duty, vvt_pwm_max_count);
